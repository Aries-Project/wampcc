...
void wamp_session::handle_HELLO(json_array& ja)
{
  ...
  challenge["authmethod"] = authmethod;
...
void wamp_session::handle_AUTHENTICATE(json_array& ja)
{
  ...
  LOG_INFO(m_log_prefix << "authenticate: " << ja);
...
void wamp_session::notify_session_open()
{
  ...
  m_promise_on_open_or_fail.set_value(true);
}
...
std::future<bool> wamp_session::hello(client_credentials cc)
{
  /* USER thread */
  ...
  return m_promise_on_open_or_fail.get_future();
}
...
std::future<bool> wamp_session::hello_common(const std::string& realm,
...
std::future<bool> wamp_session::hello(const std::string& realm)
{
...
void wamp_session::transition_to_closed()
{
  ...
  try {
    ...
    m_promise_on_open_or_fail.set_value(false);
  }
  catch (...) {
    /* ignore */
  }
